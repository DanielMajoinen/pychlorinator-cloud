...
TIMER_PUMP_SPEEDS = {"Low": 1, "Medium": 2, "High": 3}
EQUIPMENT_NAMES = [
    "PoolSpa",
    "FilterPump",
    "Heater",
...
# Live testing showed the second poll consistently sees the new values
...
RECEIVE_WATCHDOG_TIMEOUT_SECONDS = 11.0
...
def build_equipment_timer_payload(
    *,
    ...
    equipment: list[str],
    pump_speed: Literal["Low", "Medium", "High"] = "Medium",
) -> bytes:
...
def build_heat_demand_payload(
    *,
    ...
    Decompiled from the vendor app (Pack=1):
      [0] HeatDemandEnabled
...
class HaloWebSocketClient:
    ...
    def _quiet_reconnect_missing_core_cmds(self) -> list[int]:
        ...
        if self.data.operating_days is None:
    ...
    async def _request_all_data(self) -> None:
        ...
        # NOTE(2026-05-21): An experimental 'vomit handshake' that pre-sent
        # 0x006B + 0x0005 here was REVERTED in this build. it introduced a new
        # connectresp-timeout failure mode in production (a 30-minute live soak
        # showed median session 43.7s vs 79s pre-fix). Vendor parity for this
        # handshake needs proper synchronous wait on the 0x0005 response before
        # any further reads. Filed for proper implementation in a separate
        # change.
    ...
    def _assemble_valve_custom_name(self, valve_index: int) -> str | None:
        ...
        buf = self._valve_custom_name_buffer.get(valve_index)
        ...
        return (
            assembled[:target_length].decode("utf-8", errors="replace").rstrip("\x00")
        )
    ...
    async def write_heat_demand(
        ...
        *,
        ...
        # 2026-05-21: live testing showed the immediate
    ...
    async def _keepalive_loop(self) -> None:
        """Send keepalive + time poll every 2 seconds to maintain the connection.

        2026-05-21: Vendor app sends keepalive AND a 0x0002 (controller time)
        read every ~4s. The time poll appears to act as an application-level
        heartbeat that keeps the controller's session alive. Without it,
        sessions die at 13-31s even with JSON keepalive frames flowing.
        We send both every 2s (our existing cadence) to match or exceed
        the vendor's heartbeat rate.
        """
        try:
            while self._running and self._ws:
                ...
                if self._ws and self._running:
                    try:
                        ...
                        # Vendor app polls controller time alongside keepalive.
                        # This acts as an application-level heartbeat that the
                        # controller/relay uses to confirm the client is alive.
    ...
    def _update_data(self, parsed: dict[str, Any], raw: bytes) -> None:
        ...
        if parsed.get("type") == "state":
            ...
            if parsed.get("pump_speed") is not None:
                ...
                # 2026-05-21. pump_speed Medium↔High flip fix (observed
                # during live testing). The 0x0324 carousel
            ...
            self.data.operating_days = parsed.get("operating_days")
            self.data.unknown_counter_a = parsed.get("unknown_counter_a")
            ...
            self.data.today_cell_runtime_minutes = parsed.get(
                "today_cell_runtime_minutes"
            )
