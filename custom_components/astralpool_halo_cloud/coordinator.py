...
class HaloCloudCoordinator(DataUpdateCoordinator[ChlorinatorLiveData]):
    ...
    def _handle_client_disconnect(self) -> None:
        ...
        try:
            ...
            # "scheduling quiet reconnect" line that surfaces during pause
            # activation.
            ...
            reconnect_delay = _POST_DISCONNECT_RECONNECT_BACKOFF_START
            circuit_delay = self._short_session_reconnect_delay()
            if circuit_delay is not None:
                reconnect_delay = circuit_delay
                _LOGGER.warning(
                    "Cloud WebSocket has had %s short-lived sessions within %ss; "
                    "holding reconnect for %ss",
