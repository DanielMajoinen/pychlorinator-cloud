...
def parse_timer_state(data: bytes) -> dict[str, Any]:
    """Parse cmd 0x0192 TimerState.

    fw2.3 (validated against a live 0x0192 frame 2026-07-06:
    ``00 01 00 01``) uses the 4-byte TimeStateCharacteristic:
      [0] current (active) equipment timer slot
      [1] next equipment timer slot
      [2] current (active) lighting timer slot
      [3] next lighting timer slot

    Season is NOT carried here — it comes from TimerSetup (0x0191). The previous
    build derived season from byte 0 via a {1:Winter, 2:Summer} map, which
    corrupted the reported season whenever an equipment timer in slot 1 or 2 was
    active. That derivation is removed.
    """
    if len(data) < 1:
    ...
    if len(data) > 2:
        result["active_lighting_timer"] = data[2]
    if len(data) > 3:
        result["next_lighting_timer"] = data[3]
    return result
