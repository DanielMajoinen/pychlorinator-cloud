...
async def pair_via_ble(
    ...
    for attempt in range(1, 4):
        ...
        client = None
        # Per-step timing (monotonic seconds from attempt start). The
        # controller runs its own pairing-mode countdown that starts when the
        # access code is shown; if our connect+handshake overruns it the
        # controller displays "timed out" even though the GATT exchange may
        # still complete. These marks make the budget measurable so we can see
        # exactly where the time goes during a re-pair.
        timings: dict[str, float] = {}
        t0 = time.monotonic()

        def _mark(step: str) -> None:
            timings.setdefault(step, time.monotonic() - t0)

        try:
            client = await _connect_client(ble_address)
            _mark("connect")
            LOGGER.debug(
                "Connected to %s in %.2fs (attempt %d/3)",
            ...
            _mark("send_719")
            LOGGER.debug(
                "Sent username registration at +%.2fs (controller countdown is "
                "running from when its access code appeared)",
                timings["send_719"],
            )
            ...
            def _dur(a: str, b: str) -> float:
                return timings.get(b, 0.0) - timings.get(a, 0.0)

            LOGGER.info(
                "Pairing complete (%d chars) in %.2fs. Breakdown: connect=%.2fs "
                "session_key=%.2fs mac=%.2fs notify=%.2fs send719@+%.2fs "
                "first_fragment@+%.2fs. (Controller pairing countdown is the "
                "constraint — send719 should land well before it expires.)",
                len(password),
                timings.get("complete", 0.0),
                timings.get("connect", 0.0),
                _dur("connect", "session_key"),
                _dur("session_key", "mac_auth"),
                _dur("mac_auth", "notify"),
                timings.get("send_719", 0.0),
                timings.get("first_fragment", 0.0),
            )
            return password
            ...
            if attempt >= 3:
                raise
            # Fast retry: the controller's pairing window is ticking, so do NOT
            # back off for seconds (the previous `sleep(attempt)` burned 1-2s of
            # that window per retry). Reconnect immediately after a short settle.
            LOGGER.warning(
                "BLE pairing transport attempt %d/3 failed after %.2fs, retrying "
                "fast: %s",
                attempt,
                time.monotonic() - t0,
                err,
